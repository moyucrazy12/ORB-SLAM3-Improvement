...
FrameDrawer::FrameDrawer(Atlas* pAtlas):both(false),mpAtlas(pAtlas)
{
    ...
    for (int i = 0; i < 80; i++) {
        ...
        int g = rand() % 256;
        int r = rand() % 256;
        info_yolo.color.push_back(cv::Scalar(b, g, r));
    }
}
...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    DrawYOLO(imWithInfo, yolo_detection);
    return imWithInfo;
}


void FrameDrawer::DrawYOLO(cv::Mat &img, std::vector<OutputParams> result)
{
    ...
    for (int i = 0; i < result.size(); i++) { 
        if(info_yolo.classes_yolo[result[i].id]=="car"){
            ...
            if (result[i].box.area() > 0) {
                rectangle(img, result[i].box, info_yolo.color[result[i].id], 2, 8);
                left = result[i].box.x;
                top = result[i].box.y;
            }
            if (result[i].rotatedBox.size.width * result[i].rotatedBox.size.height > 0) {
                DrawRotatedBox(img, result[i].rotatedBox, info_yolo.color[result[i].id], 2);
                left = result[i].rotatedBox.center.x;
                top = result[i].rotatedBox.center.y;
            }
            if (result[i].boxMask.rows && result[i].boxMask.cols > 0)
                mask(result[i].box).setTo(info_yolo.color[result[i].id], result[i].boxMask);
            std::string label = info_yolo.classes_yolo[result[i].id] + ":" + std::to_string(result[i].confidence);
            int baseLine;
            cv::Size labelSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
            top = MAX(top, labelSize.height);
            putText(img, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 1, info_yolo.color[result[i].id], 2);
        }  
